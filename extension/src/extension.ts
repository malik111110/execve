...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const startAgentCommand = vscode.commands.registerCommand(
    ...
    async () => {
      await startSession("agent", output);
    }
  );
  ...
  const startChatCommand = vscode.commands.registerCommand(
    ...
    async () => {
      await startSession("chat", output);
    }
  );
  ...
  context.subscriptions.push(startAgentCommand, startChatCommand, legacyStartCommand);
}

async function startSession(mode: SessionMode, output: vscode.OutputChannel): Promise<void> {
  const prompt = await vscode.window.showInputBox({
    title: mode === "agent" ? "Local LLM Agent (Agent Mode)" : "Local LLM Agent (Chat Mode)",
    prompt:
      mode === "agent"
        ? "Describe what you want the local agent to do"
        : "Describe what you want to discuss in chat mode"
  });
  ...
  try {
    ...
    void vscode.window.showInformationMessage(
      `Local Agent (${mode}): ${response.finalMessage}`
    );
  } catch (error) {
